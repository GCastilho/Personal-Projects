...
is_handshake(){
	...
	if [[ ! $(($msg_timestamp+60)) < $(date +%s) ]] && [[ ! $(date +%s) < $(($msg_timestamp+60)) ]]; then
...
connect_server(){
	...
	if (receive_data); then is_handshake; else echo "Erro de conexão"; main_menu; fi
}
